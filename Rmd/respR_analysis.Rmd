...
plate_bg_rates <- rates_frame_blank %>% 
  group_by(Meas_Day, Plate) %>% 
  summarise(mean_bg_rate = mean(rate),
            se_bg_rate = se(rate)) %>% 
  unite(Plate_key, Meas_Day, Plate, sep = "_", remove = FALSE)
#
plate_bg_rates %>% 
...
rates_frame_samples <- rates_frame_samples %>% 
  ...
  select(-Meas_Day.y, -Plate.y) %>% dplyr::rename(Meas_Day = Meas_Day.x, Plate = Plate.x)
```
...
```
```{r dissertation figure 4.3A}
# Treatment 
title <- expression(paste(italic("Pocillopora"), " oxygen consumption rates", collapse = ""))
Resp_Treatment <- ggplot(rates_frame_samples, aes(Day, rate, color = Treatment, group = Treatment)) +
  ...
  stat_summary(fun.y=mean, geom="line") + 
      scale_color_manual(values = treatcolors[2:3]) +
      scale_y_continuous(name=("Oxygen consumption rate (ug O2/min)")) +
  # coord_cartesian(ylim=c(0, 6)) +
  ggtitle(title)
plot_4A <- Resp_Treatment + facet_nested(. ~ mtORF + Symbiont_Genus, scales = "free_x", space = "free", switch = NULL)
ggsave("./outputs/physio/respR/respR_progression_plot_mtorf_symbiont.pdf", device = "pdf", height = 3.5, width = 6)
```
```{r dissertation figure 4.3B}
# Treatment 
title <- expression(paste(italic("Pocillopora"), " mass-corrected oxygen consumption values", collapse = ""))
Resp_Treatment <- ggplot(rates_frame_samples, aes(Day, rate_mass_residual, color = Treatment, group = Treatment)) +
  stat_summary(fun.data = "mean_se",geom = "errorbar", width = 0.2, position = position_dodge(0.1))+
  stat_summary(fun.y=mean, geom="point", size =2, alpha=0.8, position = position_dodge(0.1)) +
  stat_summary(fun.y=mean, geom="line") + 
  ...
  # coord_cartesian(ylim=c(0, 6)) +
  ggtitle(title)
plot_4B <- Resp_Treatment + facet_nested(. ~ mtORF + Symbiont_Genus, scales = "free_x", space = "free", switch = NULL)
ggsave("./outputs/physio/respR/respR_progression_plot_mtorf_symbiont_normalized.pdf", device = "pdf", height = 3.5, width = 6)
```
```{r}
plot_4A / plot_4B
```
