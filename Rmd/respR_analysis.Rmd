...
pdf("./outputs/figures/FigS2_O2_consumption_raw.pdf", width = 6.5, height = 5)
# 
raw_resp_title <- expression(paste(italic("Pocillopora"), " microfragment oxygen consumption raw values", collapse = ""))
yaxis_title <- expression('%'~O[2]~saturation)
# plot all plates colored by treatment
raw_data_plot_treatment <- resp_all_df_tidy_meta %>% 
  filter(`Time/Min.`> 15 & `Time/Min.` < 45) %>% 
  filter(!is.na(Treatment)) %>%
  ...
  facet_grid(day ~ plate, switch = "y") +
  scale_y_continuous(limits = c(-20,120), breaks = seq(0,100, 25), name = yaxis_title, position = "right") +
  ggtitle(raw_resp_title)
print(raw_data_plot_treatment)
# 
dev.off()
```
...
#
rates_frame[i-2, 1] <- names(wells_aoi[i-2])
rates_frame[i-2, 2] <- rate$rate.output
...
rate <- plate %>%
  ...
  filter(`Time/Min.` < 45) %>% # filter after 45 minutes, usually some wells below accepted O2 minimum 
  inspect(time = 2, oxygen = i) %>%
  auto_rate(method = "linear") %>% # automatically determine most linear segment
  print()  %>%   # just a quick preview
   # Incorporate background rate detection into the analysis
  convert_rate(oxy.unit = "%Oxy",
               time.unit = "min",
...
#
rates_frame[i-2, 1] <- names(wells_aoi[i-2])
rates_frame[i-2, 2] <- rate$rate.output[1] # retrieve the most linear, representative rate
# colnames(rates_frame) <- c("Well", "rate")
}
return(rates_frame)
}
...
```{r}
pdf("./outputs/figures/FigS3_O2_background.pdf", width = 6.5, height = 3.5)
bg_title <- expression(paste(italic("Pocillopora"), " microfragment and background oxygen consumption rates", collapse = ""))
yaxis_title <- expression(raw~O[2]~consumption~rate~(mu*mol~O[2]~min^-1))
# Vizualize plate background rates along with fragment rate distribution
  ggplot(data = plate_bg_rates, aes(Plate, mean_bg_rate)) +
  geom_boxplot(data = rates_frame_samples, aes(Plate, rate), color = "darkgrey", outlier.shape = NA) +
  geom_jitter(data = rates_frame_samples, aes(Plate, rate), size = 0.2, width = 0.15) +
  geom_point(data = plate_bg_rates,
             aes(Plate, mean_bg_rate), color = "red", size = 0.2) +
  geom_errorbar(data = plate_bg_rates,
                aes(ymin = mean_bg_rate-se_bg_rate, ymax = mean_bg_rate+se_bg_rate), width = 0.15, color = "red") +
  facet_grid(~ Day) +
...
```{r summarize_corrected_rates}
rate_reduction_table <- rates_frame_samples_background_corrected %>% 
  group_by(Genotype, Treatment) %>% 
  summarise(mean_rate = mean(rate)) %>% 
  pivot_wider(names_from = Treatment, values_from = mean_rate) %>% 
  transmute(pct_rate_reduction = (1 -Antibiotics/Control)*100)
  
rate_reduction_table_days <- rates_frame_samples_background_corrected %>% 
...
```{r}
pdf("./outputs/figures/FigS4_O2_mass_normalization.pdf", width = 6.5, height = 3.5)
# 
mass_norm_title <- expression(paste("Mass normalization of ", italic("Pocillopora"), " microfragment oxygen consumption rates", collapse = ""))
xaxis_log_title <- "log10(Mass)"
yaxis_log_title <- expression(log10(raw~O[2]~consumption~rate~(mu*mol~O[2]~min^-1)))
# 
rates_frame_samples %>% 
  dplyr::group_by(Fragment_ID) %>% 
  dplyr::mutate(mean_lograte = mean(log10(rate)),
  ...
  geom_point(aes(fill  = Treatment), alpha = 0.8, shape = 21) +
  ...
  scale_fill_manual(values = treatcolors[2:3])  + 
  scale_x_continuous() +
  scale_y_continuous(name = yaxis_log_title) +
  ggtitle(mass_norm_title)
dev.off()
```
...
--> use the model residuals as the mass-corrected rates (units do not carry over)
```{r rate_mass_normalization}
# Allometric scaling of metabolic rate
# MR = aM^b
# model-adjusted scaling coefficient (b) = 0.6068693
...
rates_frame_samples <- rates_frame_samples %>%
  mutate(mass_ratio = abs(Mass/0.140035), # rates scaled to median fragment mass
         rate_norm = (rate / Mass),
         rate_norm_res = (rate * mass_ratio)^wam_model$coefficients[2])
# Rate normalization by fragment mass
# beta assumed 1
# 
rates_frame_samples %>% 
  summarise(mean_rate = mean(rate*1000),
            sd_rate = sd(rate))
```
If M = aWb
Then
...
Resp_Genotype + facet_grid(Symbiont_Genus~Treatment)
...
yaxis_title <- expression(O[2]~consumption~rate~residuals)
...
pdf("./outputs/figures/Fig2B_resp_progression_plot_mtorf_symbiont.pdf", height = 90/25.4, width = 175/25.4) # two-column max size
resp_title <- expression(paste(italic("Pocillopora"), " mass-corrected oxygen consumption rates", collapse = ""))
yaxis_title <- expression(Oxygen~consumption~rate~residuals)
# New facet label names
mtorf_labs <- c("Type 1", "Type 3")
names(mtorf_labs) <- c("Type_1", "Type_3")
sym_labs <- c(expression(italic(Cladocopium)), expression(italic(Durusdinium)))
names(sym_labs) <- c("Cladocopium", "Durusdinium")
# Treatment 
resp_plot <- ggplot(rates_frame_samples, aes(Day, rate_mass_residual, color = Treatment, fill = Treatment, group = Treatment)) +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = 0.2, position = position_dodge(0.1))+
  stat_summary(fun.y=mean, geom="point", shape = 21, size = 2, alpha = 1, stroke = 0.5, color = "black", position = position_dodge(0.1)) +
  stat_summary(fun.y=mean, geom="line") + 
      scale_color_manual(values = treatcolors[2:3]) +
      scale_fill_manual(values = treatcolors[2:3]) +
      scale_y_continuous(name = yaxis_title, breaks = seq(-0.5, 0.5, 0.1)) +
...
summary(respR_lme_1)
#
anova(respR_lme_1) # Significant fixed effect of treatment on model residualsx
#
ranova(respR_lme_1) # Significant random effect of symbiont type, no random effect of mtORF type
#
step(respR_lme_1)
```
```{r effect_plot}
...
respR_lme_2 <- lmer(rate_norm*1000 ~ Treatment * DayF + 
