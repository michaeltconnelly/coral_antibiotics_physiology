...
```
```{r all_plates_24-channel_metadata}
...
```{r}
Resp_Genotype <- ggplot(rates_frame_samples, aes (Meas_Day, rate_norm, color = Genotype, group = Genotype)) +
...
```{r}
# Treatment 
Resp_Treatment <- ggplot(rates_frame_samples, aes (Meas_Day, rate_norm, color = Treatment, group = Treatment)) +
  stat_summary(fun.data = "mean_cl_boot",geom = "errorbar", width = 0.2)+
...
ggsave("./outputs/physio/respR/respR_progression_plot_its2_treatment.pdf", device = "pdf", height = 5, width = 11)
```
```{r}
...
resp_progression_boxplot_genotype_its2 <- rates_frame_samples %>% 
  ...
  # filter(Gulf == "Chiriqui") %>%
  # filter(Gulf == "Panama") %>%
  ggplot(aes(Meas_Day, rate_norm, group = Genotype, color = Genotype)) +
