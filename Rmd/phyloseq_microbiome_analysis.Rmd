...
# sample_data() Sample Data:       [ 184 samples by 11 sample variables ]
...
# write_csv(colors_families, "./Rmd/taxa_colors/colors_families_defense.csv")
# # Genera
...
pdf("./outputs/figures/Fig3_ABC.pdf", height = 91.4/25.4, width = 175/25.4)
print(figure_3)
dev.off()
```
...
```{r core_microbiomes}
# 
core_taxa_standard <- core_members(ps_filtered_rel, detection = 0, prevalence = 80/100)
# 
core_taxa_fifty <- core_members(ps_filtered_rel, detection = 0, prevalence = 50/100)
# 
```
```{r core_microbiome_abundance}
core_abundance_standard <- sample_sums(core(ps_filtered_rel, detection = 0, prevalence = .8))
core_abundance_fifty <- sample_sums(core(ps_filtered_rel, detection = 0, prevalence = .5))
```
### Plots of core microbiome relative abundance
```{r}
core_abundance_df <- data.frame(core_abundance_fifty) %>% 
  rownames_to_column("Sample_ID") %>% 
  dplyr::rename("core_abundance" = core_abundance_fifty) %>% 
  left_join(sample_metadata, by = "Sample_ID")
```
```{r}
core_abundance_df %>% 
  ggplot(aes(Genotype, core_abundance)) +
  facet_grid(~ mtORF, space = "free", scales = "free") +
  geom_boxplot()
```
...
# create phyloseq objects for core microbiome ASVs
ps_filtered_rel_0.05 <- filter_taxa(ps_filtered_rel, function(x) sum(x) > 0.05, TRUE)
# 
ps_filtered_core_standard <- prune_taxa(core_taxa_standard, ps_filtered_rel)
ps_filtered_core_standard
# 
ps_filtered_core_fifty <- prune_taxa(core_taxa_fifty, ps_filtered_rel)
...
```
```{r}
# bubble_0.01 <-  bubble_data %>% dplyr::filter(relative_abundance > 0.01)
# bubble_0.05 <-  bubble_data %>% dplyr::filter(relative_abundance > 0.05)
...
bubble_core_fifty$Class <- factor(bubble_core_fifty$Class, 
                                  levels = c(" Alphaproteobacteria", " Gammaproteobacteria", " Desulfovibrionia",
                                             " Planctomycetes", " Pla4_lineage",
                                             " Phycisphaerae",
                                             " Holophagae", " Desulfobacteria",
                                             " Bdellovibrionia", " Spirochaetia", " Campylobacteria"), 
                                  ordered = TRUE)
```
...
ps_tree <- phy_tree(ps_filtered_core_fifty) # ps_filtered_core_fifty
p <- ggtree(ps_tree)
...
all(rownames(taxa_df_core_fifty) == ps_tree$tip.label) 
# change tip labels on phylogenetic tree
ps_tree$tip.label <- taxa_df_core_fifty$Family
...
```
```{r core_asvs_bubbleplot}
bubble_core_standard$ASV <- factor(bubble_core_standard$ASV, levels = ps_tree$tip.label, ordered = TRUE)
# 
bubbleplot <- bubble_core_standard %>% ggplot(aes(x = Sample_ID, y = ASV, fill = Family)) +
  geom_point(aes(size = relative_abundance*100), alpha = 0.5, shape = 21) +
  facet_nested(. ~ Treatment + mtORF, scales="free", space="free") +
  scale_fill_brewer(palette = "Set3") +
  # scale_y_discrete(limits = levels(ps_tree$tip.label)) +
  labs(size = "Relative Abundance", fill = "Family") +
  scale_size_continuous(range = c(1, 10), breaks = c(0.01, 50, 100), limits = c(0.01, 100)) +
...
# Bubble plot showing differential effects of antibiotics treatment on bacteria taxa
pdf("./outputs/figures/Fig3D_bubbleplot_tree_all.pdf", height = 127/25.4, width = 185/25.4)
bubble_title <- expression(paste("Relative abundance of bacteria ASVs present in at least 50% of ", italic("Pocillopora"), " samples"))
# 
tree_bubbleplot <- p + 
  # geom_tiplab(size = 2) +
...
colnames(sample_asv_data) <- gsub("^X", "", colnames(sample_asv_data))
```
```{r bacteria_families_WGCNA, message=FALSE}
# need to filter to only samples analyzed for transcriptomes
family_abundances <- ps_filtered_rel %>%
  subset_samples(., sample_names(ps_filtered_rel) %in% samples$Sample_ID[match(rownames(coldata), samples$Sample_ID)]) %>% 
  tax_glom(., taxrank = "Family") %>% 
  psmelt(.) %>%
  # filter(Abundance > 0.10) %>% 
  filter(OTU %in% core_taxa_fifty)
# If you want to focus on interesting family-level relative abundances
int_families <- c("Endozoicomonadaceae", "Rhodobacteraceae", "Alteromonadaceae") #"Amoebophilaceae", "Vibrionaceae"
# 
family_average_abundances <- family_abundances %>%
  # filter(Family %in% int_families) %>%
  group_by(Sample, Family) %>%
  dplyr::summarise(avg_rel_abundance = mean(Abundance, na.rm = TRUE))
# 
datTaxa_Family <- family_average_abundances %>% 
