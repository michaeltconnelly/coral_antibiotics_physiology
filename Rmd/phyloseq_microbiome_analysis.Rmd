...
#
pca_clr_biplot
ggsave(pca_clr_biplot, filename = "./outputs/phyloseq_results/figures/clr_ggbiplot_decontam.pdf", width = 6.5, height = 5, units = "in", device = "pdf")
```
```{r clr_ggbiplot_treatment_genotype}
pca_clr_biplot <- d_pcx %>% 
  gg_biplot(., choices = c(1,2),
            groups = sample_metadata$haplotype,
...
beta_boxplot <- mod_distance %>%
  ...
  geom_jitter(color = "black", fill = "black", width = 0.05, shape = 1, size = 1, stroke = 1, show.legend = TRUE) +
  ...
  ggtitle("Beta diversity: distance to centroid boxplots") +
  theme(legend.position = "none",
          axis.text.x = element_text(angle = 0),
        strip.background = element_blank(),
...
obs_box <- ps_filtered_samples %>% plot_richness(., x="Treatment", measures=c("Observed")) +
  ...
  ggtitle("Alpha diversity: richness boxplots") +
  theme_bw() +
  theme(
          axis.text.x = element_text(angle = 0),
        strip.background = element_blank(),
...
```
```{r}
pca_clr_biplot | obs_box / beta_boxplot 
ggsave("~/Desktop/bacteria-diversity.pdf", device = "pdf", height = 6, width = 12)
```
...
## Core microbiome
```{r}
core.taxa.standard <- core_members(ps_filtered_rel, detection = 0, prevalence = 80/100)
```
```{r}
...
tax_table(ps_filtered_rel) %>% View()
```
```{r}
ps_filtered_rel_0.05 <- filter_taxa(ps_filtered_rel, function(x) sum(x) > 0.05, TRUE)
# 
sample_asv_data <- data.frame(otu_table(ps_filtered_rel_0.05)) %>% rownames_to_column("Sample_ID") 
colnames(sample_asv_data) <- gsub("^X", "", colnames(sample_asv_data))
```
```{r create tidy data frame for bubble plots}
bubble_data <- left_join(sample_metadata, sample_asv_data, by = "Sample_ID") %>%
  pivot_longer(cols = -(Sample_ID:mtORF), names_to = "ASV", values_to = "relative_abundance") %>%
  full_join(taxa_df_filtered, by = "ASV") %>% arrange(desc(relative_abundance))
# 304,610 obs. of 21 variables
```
```{r}
# bubble_0.01 <-  bubble_data %>% dplyr::filter(relative_abundance > 0.01)
# bubble_0.05 <-  bubble_data %>% dplyr::filter(relative_abundance > 0.05)
bubble_core <- bubble %>% dplyr::filter(ASV %in% core.taxa.standard)
```

```{r}
# One thing I want to do is havve the phylogenetic tree next to the plot
bubble_data %>% ggplot(aes(x = Sample_ID, y = ASV, fill = Genus)) +
...
```
```{r}
ps_filtered_rel_0.05 %>% plot_tree()
```
