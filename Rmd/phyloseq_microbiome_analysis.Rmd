...
core_taxa_standard <- core_members(ps_filtered_rel, detection = 0, prevalence = 0.8)
# 
core_taxa_fifty <- core_members(ps_filtered_rel, detection = 0, prevalence = 0.5)
# 
...
# 
  ...
  dplyr::count(Class) %>% View()
# 
phyloseq::otu_table(ps_filtered_core_fifty)
```
### Supplementary Table 2
```{r}
core_taxa_fifty_table <- tax_table(ps_filtered_core_fifty) %>% as.data.frame() %>% 
  rownames_to_column("ASV")
# 
write_csv(core_taxa_fifty_table, "./outputs/phyloseq_results/TableS2_core_taxa_fifty_table.csv")
# 
```
