...
library("eulerr")
```
```{r visualization packages, include = FALSE}
...
tree <- phyloseq::read_tree("./outputs/qiime2/export_silva/tree.nwk")
```
```{r factors}
# factor levels
...
# #remove OTU not seen more than one in at least 5% of samples
ps_filtered <- filter_taxa(ps_assigned, function(x) sum(x > 1) > (2), TRUE)
ps_filtered
```
```{r keep_unfiltered}
# or keep unfiltered 
...
checkNumZerosCol <- apply(otu,2,function(x) sum(x==0))
cases <- which(checkNumZerosCol == (nrow(otu) - 1))
length(cases) # 1797 columns are all zeros but one positive value
zcomp <- cmultRepl(otu[,-cases]) # GBM imputation without them works
```
...
plot_title <- expression(paste(italic("Pocillopora"), ' bacterial communities PCA', sep = ""))
pca_clr_biplot <- d_pcx %>% 
  gg_biplot(., choices = c(1,2),
            groups = sample_metadata$Treatment,
            groups2 = sample_metadata$Treatment,
  ...
  # scale_shape_manual(values = shapes, name = "Location") +
  ggtitle(plot_title) +
  guides(fill = guide_legend(title = NULL, nrow = 3), color = FALSE) +
  theme(legend.position = c(0.86, 0.845), legend.margin = margin(0,0,0,0, "mm"), legend.spacing.y = unit(1, "mm"))
#
pca_clr_biplot
# dev.off()
```
...
genera_antibiotics <- toptaxa_summary(ps_antibiotics, "Genus")
```
...
figure_3 <- (pca_clr_biplot | obs_boxplot / beta_boxplot) + 
  plot_layout(widths = c(1.4,1), guides = "keep")
