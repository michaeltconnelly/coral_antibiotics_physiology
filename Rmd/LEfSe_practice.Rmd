...
```{r treatment_lefse, message = F}
sample_data(ps_filtered_samples)
mm <- lefse(ps_filtered_samples, class = "Treatment",
      multicls_strat = TRUE)
```
```{r}
lefse_bar_treatment <- plot_ef_bar(mm, label_level = 1) +
  scale_fill_manual(values = c("Baseline" = treatcolors[1], "Control" = treatcolors[2], "Antibiotics" = treatcolors[3]))
lefse_bar_treatment
ggsave("./outputs/phyloseq_results/figures/lefse_bar_treatment.pdf", height = 30, width = 8.5, units = "in", device = "pdf")
```
## Haplotype variation
```{r haplotype_lefse, message = F}
mm_haplotype <- lefse(ps_filtered_samples, class = "haplotype",
      multicls_strat = TRUE)
```
```{r}
lefse_bar_haplotype <- plot_ef_bar(mm, label_level = 1) +
  scale_fill_manual(values = c("Baseline" = treatcolors[1], "Control" = treatcolors[2], "Antibiotics" = treatcolors[3]))
lefse_bar_haplotype
```
