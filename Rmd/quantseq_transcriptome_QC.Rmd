---
title: "quantseq transcriptome QC"
...
QC_summary$Treatment <- factor(QC_summary$Treatment, levels = treatment_levels, ordered = TRUE)
```
```{r}
# pick top aligned samples within each genotype
QC_summary %>% 
  dplyr::filter(pipeline_step == "Uniquely Mapped Reads" & Treatment != "Antibiotics") %>% 
  group_by(Genotype) %>% # (Treatment)
  dplyr::slice_max(read_count) %>%
  arrange(Genotype)
```
...
```{r}
pdf("./outputs/figures/FigS8_readcounts.pdf", width = 6.5, height = 7.5)
qcfig <- (pipeQC_counts / pipeQC_props & theme(legend.position = "right", legend.margin = margin(2,2,2,10, unit = "mm"))) + plot_layout(guides = "collect") 
...
## Symbiont type alignment rates
```{r}
sym_qcs <- read_csv("./data/SymQCs_040722.csv")
```
```{r}
sym_qc_meta <- sym_qcs %>%
  pivot_longer(., cols = c(Cladocopium, Durusdinium), names_to = "symbiont", values_to = "percent") %>%
  left_join(sample_metadata)

pdf("./outputs/figures/sym_proportion_aligned.pdf", width = 11, height = 4.5)
sym_qc_meta %>% 
  ggplot(aes(Sample_ID, percent)) + geom_col(aes(fill = symbiont)) + 
