...
```{r import_alignment}
data <- read.dna("./data/mtorf/mtorf_alignment_828bp_rsmas_corrected.fasta", format = "fasta")
checkAlignment(data)
#14 sequences, 828 sites, no gaps
# attributes(data)
# dimnames(data)[[1]]
```
```{r haplotypes}
...
#   dplyr::select(-Freq) %>%
#   dplyr::mutate(mtORF = ifelse(haplotype %in% c("II", "IV"), "Type_3", "Type_1"))
```
