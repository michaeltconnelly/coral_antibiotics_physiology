...
keep <- rowSums(counts(dds_coral) >= 10) > (ncol(countdata_sorted_coral)*0.9) # 5,537 genes
# Option 2: Remove genes with mean counts less than 10
# keep <- rowMeans(counts(dds_coral)) > 10 # 13,479 genes
# Option 3: Remove genes with mean counts less than 3
# keep <- rowMeans(counts(dds_coral)) > 3 # 17,150 genes
...
dev.off()
```
```{r figure 6A: manuscript}
pdf("./outputs/figures/Fig4A_transcriptome_pca.pdf", height = 88/25.4, width = 120/25.4)
pca_title <- expression(paste(italic("Pocillopora"), " gene expression PCA plot", sep = ""))
# 
tx_pca <- ggPCA(vsd_coral, samples, genocolors, pclab = c(1,2), ntop = ntop) +
  # facet_wrap("Genotype") +
  ggtitle(pca_title)
print(tx_pca)
dev.off()
...
### Figure 4B
```{r total_variance figure: manuscript}
pdf("./outputs/figures/FigS9_variance_partition_totals.pdf", height = 4, width = 6.5)
...
```
### Figure 4AB
```{r figure_4ab: manuscript}
figure_4ab <- tx_pca + var_sum_plot
# Need to add plot annotation tags (ie. A,B,C in Inkscape)
pdf("./outputs/figures/Fig4_AB.pdf", height = 101.6/25.4, width = 175/25.4)
print(figure_4ab)
dev.off()
```
...
adjacency <- adjacency(datExpr,
      # Network construction arguments:  correlation, adjacency function,  and topological overlap map options
                       ...
                       power = sft$powerEstimate, # 4 for coral, unknown for symbiont
                       type = "signed") # signed
# 
...
# merge cutHeight = 0.15 because I want to group together modules with >85% similar module eigengene expression
mergedMods <- mergeCloseModules(datExpr, dynamicColors, cutHeight = 0.15, verbose = 1)
# The merged module colors
mergedColors <- mergedMods$colors
table(mergedColors) 
```

### Figure 4C / Supplementary Figure 10
```{r dendrogram_modules}
pdf(file = "./outputs/figures/Fig4C_WGCNA_dendrogram_coral.pdf", width = 175/25.4, height = 50.8/25.4)
...
modP_bacteria <- as.data.frame(moduleTaxaPvalue) %>%
  ...
  select_if((~ any(. < 0.05))) # select only taxa that have a significantly correlated module
#
...
kMEfiles 
###
intModules_coral <- c("greenyellow","blue", "turquoise",  "purple", "tan", "green", "yellow", "black", "pink")
...
me_physiology_heatmap <- Heatmap(modulePhysCor,
                      col = col_fun_cor,
                      cell_fun = function(j, i, x, y, width, height, fill) {grid.text(sprintf("%s", textMatrix_phys[i, j]), x, y, gp = gpar(fontsize = 6))},
                      cluster_rows = hclustME,
                      # top_annotation = treatment_annotation,
                      # left_annotation = module_annotation,
                      # right_annotation = module_barplot,
...
me_bacteria_heatmap <- Heatmap(moduleTaxaCor,
                      col = col_fun_cor,
                      cell_fun = function(j, i, x, y, width, height, fill) {grid.text(sprintf("%s", textMatrix_taxa[i, j]), x, y, gp = gpar(fontsize = 6))},
                      cluster_rows = hclustME,
...
```
```{r}
pdf("./outputs/figures/Fig4D_wgcna_complexheatmap_option1.pdf", width = 175/25.4, height = 101.6/25.4)
draw(heatmap_combined,
     ht_gap = unit(2.5, "mm"),
     show_annotation_legend = FALSE, heatmap_legend_side = "left"
     )
draw(cor_lgd,
     x = unit(15, "mm"), y = unit(15, "mm"), just = c("left", "bottom"))
dev.off()
```
