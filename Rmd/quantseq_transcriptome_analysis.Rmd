...
pdf("./outputs/figures/Fig6A_transcriptome_pca.pdf", height = 88/25.4, width = 120/25.4)
pca_title <- expression(paste(italic("Pocillopora"), " gene expression PCA plot", sep = ""))
# 
ggPCA(vsd_coral, samples, genocolors, pclab = c(1,2), ntop = ntop) +
  # facet_wrap("Genotype") +
...
```
```{r pcatools_scree}
screeplot(pca_vsd, axisLabSize = 10, titleLabSize = 14)
```
...
var_title <- expression(paste("Factors affecting gene expression variance"))
# 
varpct %>% 
...
pdf(file = "./outputs/figures/FigS10_WGCNA_dendrogram_coral.pdf", width = 6.5, height = 4)
...
#
datTraits_Phys <- cbind(datTraits_Ipam, datTraits_Resp) # incorrect sample order
```
...
datTaxa_Family <- read_csv("./outputs/phyloseq_results/datTaxa_Family.csv")
...
me_treatment_heatmap <- Heatmap(moduleTraitCor,
                      col = col_fun_cor,
                      cell_fun = function(j, i, x, y, width, height, fill) {grid.text(sprintf("%s", textMatrix_treatment[i, j]), x, y, gp = gpar(fontsize = 8))},
...
me_physiology_heatmap <- Heatmap(modulePhysCor,
                      col = col_fun_cor,
                      cell_fun = function(j, i, x, y, width, height, fill) {grid.text(sprintf("%s", textMatrix_phys[i, j]), x, y, gp = gpar(fontsize = 8))},
                      cluster_rows = hclustME,
                      # top_annotation = treatment_annotation,
                      left_annotation = module_annotation,
                      # right_annotation = module_barplot,
