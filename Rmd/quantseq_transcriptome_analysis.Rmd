...
samples <- sample_metadata %>%
  dplyr::filter(Genotype %in% c("PAN-78", "PAN-79", "PAN-83")) %>% 
...
coldata <- coldata %>% 
  ...
  filter(Genotype %in% c("PAN-78", "PAN-79", "PAN-83"))
countdata_sorted_coral <- countdata_sorted_coral %>% 
  dplyr::select(!contains("Baseline")) %>% 
  dplyr::select(!contains(c("PAN-32", "PAN-39", "PAN-43", "PAN-37", "PAN-44", "PAN-34", "PAN-35", "URA-51")))
```
...
pca_coral <- ggPCA(vsd_coral, samples, treatcolors, pclab = c(1,2))
# pca_symbiont <- ggPCA(vsd_symbiont, samples, condcolors_AxH, pclab = c(1,2))
pca_combined <- pca_coral + pca_symbiont
# print(pca_combined)
# ggsave(pca_coral, filename = "./outputs/DESeq_results/figures/pca_coral.pdf", width = 200, height = 100, units = "mm", device = "pdf")
print(pca_coral)
```
...
```
```{r Figure_1}
# Save Figure 1 output for two-column width figure (169 mm)
ggsave(pcoa_combined, filename = "./manuscript_figures/Fig1A_PCoA_Treatment_Combined.pdf", width = 220, height = 90, units = "mm", device = "pdf")
```
