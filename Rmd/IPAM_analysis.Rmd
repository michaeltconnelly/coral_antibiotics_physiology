...
YII_Genotype + ylim(0.25, 0.7) + facet_grid(Symbiont_Genus~Treatment)
...
YII_Treatment + facet_wrap(~Symbiont_Genus) + ggtitle(title)
...
pdf("./outputs/figures/ipam_progression_plot_mtorf_symbiont_no-bones.pdf", height = 5, width = 5.75)
ipam_title <- expression(paste(italic("Pocillopora"), ' symbiont photochemical efficiency', sep = ""))
# New facet label names
mtorf_labs <- c("Type 1", "Type 3")
names(mtorf_labs) <- c("Type_1", "Type_3")
# make plot
ipam_plot <- ipam_fragments %>%
  ggplot(aes(DayF, mean_value, colour = Treatment, group = Treatment)) +
  stat_summary(fun.data = "mean_se",geom = "errorbar", width = 0.3, position = position_dodge(0.1))+
  ...
  facet_nested(. ~ mtORF + Symbiont_Genus,
               ...
               labeller = labeller(mtORF = mtorf_labs)) +
...
dev.off()
```
```{r figure 2A: manuscript}
pdf("./outputs/figures/Fig2_ipam_progression_plot_mtorf_symbiont.pdf", height = 90/25.4, width = 175/25.4) # two-column max size
# pdf("./outputs/figures/Fig2_ipam_progression_plot_mtorf_symbiont.pdf", height = 88/25.4, width = 88/25.4) # one-column max size
ipam_title <- expression(paste(italic("Pocillopora"), ' symbiont photochemical efficiency', sep = ""))
# New facet label names
mtorf_labs <- c("Type 1", "Type 3")
names(mtorf_labs) <- c("Type_1", "Type_3")
# make plot
ipam_plot <- ipam_fragments %>%
  ggplot(aes(DayF, mean_value, color = Treatment, fill = Treatment, group = Treatment)) +
  stat_summary(fun.data = "mean_se",geom = "errorbar", width = 0.2, position = position_dodge(0.1))+
...
```
#### Model #1: Treatment x Day, random mtORF, sybmiont type effects
```{r lme model 1}
...
# Significant random effects of mtORF type and symbiont type
```
```{r lme_1 effect plot}
...
## Fv/Fm trait matrix for WGCNA correlation
```{r}
# start with day 5, but process sampling data to get as close to preservation time as possible for correlations to gene expression
ipam_traits <- ipam_fragments %>% 
  filter(Sample_ID %in% samples_wgcna) %>% 
  filter(Day == "5") %>% 
  dplyr::select(Genotype, Treatment, Fragment_ID, mean_value) %>%
  dplyr::mutate("Sample" = str_c(Genotype, Treatment, Fragment_ID, sep = "_")) %>% 
  dplyr::select(-Genotype, -Treatment, -Fragment_ID) 
# 
write_csv(ipam_traits, "./outputs/physio/ipam_wgcna_traits.csv")
```
