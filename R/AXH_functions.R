...
theme_mec <- function(base_size = 10, base_family = "ArialMT") {
...
ggPCA <- function(vsd, samples, condcolors, ntop = 500,  pclab = c(1,2)) {
  #
  PCAtmtdata <- plotPCA.custom(vsd, intgroup = c("Genotype", "Treatment"), ntop = 500, returnData = TRUE,  pcs = c(pclab[1],pclab[2]))
  #set factor orders 
  ...
  PCAplot <-  PCAtmtdata %>% ggplot(aes(PC1,PC2)) +
    geom_point(size=4, aes(color = Genotype, shape = Treatment), stroke = 0.5, show.legend = TRUE) +
    xlab(paste0( "PC", pclab[1], ": ", PCAtmtpercentVar[pclab[1]], "% variance")) + 
    ylab(paste0( "PC", pclab[2], ": ", PCAtmtpercentVar[pclab[2]], "% variance")) + 
    coord_fixed(1) + 
    scale_color_manual(values=genocolors, name="Treatment") +
...
ggPCoA <- function(mds) {
  ...
  PCoAplot <- ggplot(mds, aes(fill = Treatment)) +
    ...
    # sample points
    geom_point(size = 2, aes(x = `1`, y = `2`, color = Genotype, fill = Treatment), shape = 21, stroke = 2, show.legend = TRUE) + #FALSE
    scale_color_manual(values=genocolors, name="Genotype") +
    scale_shape_manual(values=c(21,22), name="Treatment") +
    scale_fill_manual(values = c(treatcolors[3], treatcolors[2])) +
    labs(x = xlab, y = ylab) +
    coord_fixed(1) #+
